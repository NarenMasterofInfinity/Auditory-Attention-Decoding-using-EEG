#!/usr/bin/env python3
"""
model.py — AAD with TemporalStem + GraphEncoder + Cross-Attn (helper.load_subject)

- Loads one or more subjects via helper.load_subject(SX)
- Windows continuous streams; labels each window A/B using attended==env_left/right
- EEG encoder: TemporalStem -> (lift) -> GraphEncoder  (spatial over channels per time)
- Audio encoder: causal TCN for A (left) and B (right) broadband envelopes
- Fusion: Multi-Head Cross Attention (EEG queries vs audio keys/values)
- Head: binary classifier (idx0=B, idx1=A)
- Saves per-epoch logs, training curves, adjacency heatmaps, edge plots, topomaps, and test probability timeline.

Requirements: numpy, torch, matplotlib, mne, sklearn
"""
...
# --- your modules ---
import helper  # expects helper.DATASET_DIR and helper.load_subject(subject_id)

try:
    # Your uploaded file with the stem (depthwise+pointwise conv over time)
    from FeatureExpander import TemporalStem  # :contentReference[oaicite:2]{index=2}
except Exception:
    raise ImportError("FeatureExpander.py (TemporalStem) not found")

try:
    # Your uploaded GraphEncoder (anatomical prior + low-rank residual + edge-wise GATv2)
    from GraphEncoder import GraphEncoder      # :contentReference[oaicite:3]{index=3}
except Exception:
    raise ImportError("GraphEncoder.py not found")
...
def make_broadband(env):
    """env [T,B] -> mean over bands [T]."""
    if env.ndim == 2: return env.mean(axis=1)
...
def make_biosemi64_info(n_ch=64, sfreq=128.0):
    if n_ch == 64:
...
class AADDataset(Dataset):
    """
    Returns: eeg_win [W,C], envA_win [W], envB_win [W], label (0/1, 1→A=left)
    """
    def __init__(self, eeg, envA, envB, att_vec, fs, win_sec, hop_sec, zscore_per_window=True):
        ...
        # labels per sample: 1 if attended==left else 0 (B)
        # att_vec here is broadband attended [T]; we compare closeness to left/right broadband
        # Build per-sample 0/1 array, then majority vote inside each window
        is_A = np.isclose(att_vec, self.a, rtol=1e-5, atol=1e-6).astype(np.int64)  # [T]
        labs = []
        for s, e in self.spans:
            labs.append(maj_vote_label(is_A[s:e]))
        self.labels = np.array(labs, dtype=np.int64)
        self._zwin = zscore_per_window
    ...
    def __getitem__(self, i):
        ...
        if self._zwin:
            X = zscore_window(X)
        return X, self.a[s:e], self.b[s:e], self.labels[i]
...
class EEGEncoder(nn.Module):
    """
    TemporalStem → Linear(lift to 127) → concat raw EEG channel scalar → GraphEncoder
    ...
    def __init__(self, n_ch, pos, d_stem=256, d_lift=127, d_in=128, d_model=128,
        ...
        super().__init__()
        self.stem = TemporalStem(in_ch=n_ch, out_ch=d_stem, causal=causal, dropout=dropout)  # :contentReference[oaicite:4]{index=4}
        self.lift = nn.Linear(d_stem, d_lift)
        self.graph = GraphEncoder(pos=pos, d_in=d_in, d_model=d_model, L=L, k=k, heads=heads, dropout=dropout)  # :contentReference[oaicite:5]{index=5}
...
class AudioTCN(nn.Module):
    """Causal depthwise-separable TCN on a 1D envelope."""
...
class CrossAttention(nn.Module):
    """Pre-norm cross-attn: EEG queries, Audio keys/values."""
    ...
    def forward(self, eeg_tok, aud_tok):
        ...
        ctx = self._merge(ctx)
        out = self.ln_out(eeg_tok + self.do(self.proj(ctx)))
        return out

class AADModel(nn.Module):
    """
    EEG encoder + two Audio TCNs + cross-attn (A/B) + classifier
    logits[:,0] == B, logits[:,1] == A
    """
    def __init__(self, n_ch, pos, d_model=128, d_audio=64, L=3, k=8, heads_graph=4, heads_xattn=4, dropout=0.1):
        super().__init__()
        self.eeg_enc = EEGEncoder(n_ch=n_ch, pos=torch.tensor(pos, dtype=torch.float32),
                                  d_model=d_model, L=L, k=k, heads=heads_graph, dropout=dropout)
    ...
    def forward(self, eeg, envA, envB, bt_chunk=None):
        ...
        logits = torch.stack([logitB, logitA], dim=-1)
        return logits, A
...
def train_one_subject(subj_id_str, epochs, batch, win_sec, hop_sec, lr, device,
                      outdir, logs_root, k, heads_graph, blocks_graph, heads_xattn, bt_chunk, amp,
                      workers, prefetch, accum, compile_flag,
                      patience, min_delta, warmup_pct, final_lr_pct):

    ensure_dir(outdir)

    # ----- load continuous streams via helper.load_subject -----
    pack = helper.load_subject(subj_id_str)
    eeg, envL, envR, att_env, fs = pack["eeg"], pack["env_left"], pack["env_right"], pack["attended"], pack["fs"]

    # collapse band envelopes to broadband
    A_broad = make_broadband(envL)  # left == A
    B_broad = make_broadband(envR)  # right == B
    Att_broad = make_broadband(att_env)

    # z-score EEG globally then per window (keeps training stable)
    X = eeg.astype(np.float32)
    X, _, _ = zscore_global(X)

    n_ch = X.shape[1]
    info, ch_names, pos = make_biosemi64_info(n_ch=n_ch, sfreq=fs)

    # dataset + splits
    ds = AADDataset(X, A_broad, B_broad, Att_broad, fs, win_sec, hop_sec, zscore_per_window=True)
    ...
    tr_idx, va_idx, te_idx = split_indices(len(ds), 0.7, 0.1, seed=2000 + int(subj_id_str.strip('S')))
    class _Sub(Dataset):
    ...
    train_ds, val_ds, test_ds = _Sub(ds, tr_idx), _Sub(ds, va_idx), _Sub(ds, te_idx)

    pinmem = (torch.cuda.is_available() and str(device).startswith('cuda'))
    use_mp = (workers > 0)
    ...
    train_ld = DataLoader(train_ds, batch_size=batch, shuffle=True, drop_last=True,
                          num_workers=workers, pin_memory=pinmem, persistent_workers=use_mp, **prefetch_kw)
    ...
    # per-subject CSV in Logs/
    csv_path = os.path.join(logs_root, f'{subj_id_str}_train_log.csv')
    ...
    for ep in range(1, epochs + 1):
        ...
        print(f"{subj_id_str} Ep{ep:02d} | train {tr_loss:.4f} acc={tr_acc:.3f} | val {va_loss:.4f} acc={va_acc:.3f} | {sec:.1f}s")
        ...
        if improved:
            ...
            wait += 1
            if wait >= patience:
                print(f"{subj_id_str} Early stopping at epoch {ep} (best val {best_val:.4f})")
                break
    ...
    print(f"{subj_id_str} Final: Val acc={val_acc:.3f} AUC={val_auc:.3f} | Test acc={test_acc:.3f} AUC={test_auc:.3f}")

    # graphs on one batch
    with torch.no_grad():
        loader_pick = val_ld if len(val_ld) > 0 else test_ld
        xb, a, b, yb = next(iter(loader_pick))
    ...
    # save A0 and delta visuals (if available)
    try:
...
def parse_subjects(s):
    # Accept "S1-S16" or "S1,S4,S7"
    s = s.strip()
    if '-' in s:
        a, b = s.split('-')
        a = a.strip().upper().lstrip('S'); b = b.strip().upper().lstrip('S')
        return [f"S{i}" for i in range(int(a), int(b)+1)]
    else:
        parts = [p.strip().upper() for p in s.split(',') if p.strip()]
        return [p if p.startswith('S') else f"S{int(p)}" for p in parts]

def main():
    p = argparse.ArgumentParser()
    p.add_argument('--subjects', type=str, default='S1-S16', help='e.g., "S1-S16" or "S1,S3,S5"')
    p.add_argument('--outdir', type=str, default='AAD_out')
    p.add_argument('--epochs', type=int, default=100)
    p.add_argument('--batch', type=int, default=8)
    p.add_argument('--win_sec', type=float, default=8.0)
    p.add_argument('--hop_sec', type=float, default=4.0)
    ...
    logs_root = ensure_dir(os.path.join(args.outdir, "Logs"))
    ...
    subs = parse_subjects(args.subjects)

    for subj_id in subs:
        sx = ensure_dir(os.path.join(args.outdir, subj_id))
        print(f"\n=== {subj_id} → {sx} ===")
        val_acc, val_auc, test_acc, test_auc = train_one_subject(
            subj_id, args.epochs, args.batch, args.win_sec, args.hop_sec, args.lr, args.device,
            sx, logs_root, args.k, args.heads_graph, args.blocks_graph, args.heads_xattn, args.bt_chunk, args.amp,
            args.workers, args.prefetch, args.accum, args.compile, args.patience, args.min_delta,
            args.warmup_pct, args.final_lr_pct
        )
        with open(summ_path, 'a', newline='') as f:
            csv.writer(f).writerow([subj_id, val_acc, val_auc, test_acc, test_auc])
        print(f"{subj_id}: Val acc={val_acc:.3f} AUC={val_auc:.3f} | Test acc={test_acc:.3f} AUC={test_auc:.3f}")

    print(f"\nWrote {summ_path} and per-epoch logs under {logs_root}")
