...
# ---------- Custom arrays ----------
full_architecture = np.array([
    0.5821, 0.5593, 0.5315, 0.5407, 0.5634, 0.5498,
    0.5572, 0.5426, 0.5344, 0.5610, 0.5448, 0.5555,
    0.5299, 0.5522, 0.5388, 0.5194, 0.4986, 0.4760
])
...
# Add both to models BEFORE preparing labels/data
models["GraphEncoder"] = graphencoder_vals
models["Proposed (Full Architecture)"] = full_architecture

# ---------- 2) Prepare plot data ----------
# Keep a sensible display order (baselines -> TRF/CCA -> GraphEncoder -> Proposed)
preferred_order = [
    "Linear (OLS)", "Ridge (Tikhonov)", "Lasso", "ElasticNet",
    "TRF", "CCA", "GraphEncoder", "Proposed (Full Architecture)"
]
labels = [k for k in preferred_order if k in models]
...
# ---------- 4) Plot B: Combined box plot (now includes Proposed Full) ----------
...
fig.savefig(out_dir / "box_plot.png", dpi=300)
fig2.savefig(out_dir / "box_all_models_full_plot.png", dpi=300)
